...
DayOrder 当前从环境变量读取 PostgreSQL 连接信息，并为 API、Worker 和 Migrator 使用三种不同的数据库角色。当前目录已经配置 ConfigHub CLI，本地 `.confighub.yaml` 可以访问 `https://config.shier.art` 的 `shier/prod`。ConfigHub Revision 2 提供以下非空字段：
...
通过 `confighub run --project shier --env prod -- <command>` 将四个字段注入进程。DayOrder 新增一个聚焦于 PostgreSQL 连接派生的内部组件，用于验证字段、构造管理员和运行角色 DSN，并在读取后清理原始敏感环境变量。
...
完整 DSN 可以让 DayOrder 零适配，但会重复地址、端口和密码，轮换时需要同时修改多个值，也不符合本次固定数据库名的要求。
...
不让三个角色直接复用管理员密码。每个角色密码按以下稳定规则派生：

```text
base64url(HMAC-SHA256(db_password, "dayorder-db-role/v1/<role>"))
```

派生结果不写入文件、不输出到终端，也不提交到 ConfigHub。相同主密钥可以在 Bootstrap 和运行时得到相同角色密码；不同角色得到不同密码。主密钥轮换时，必须先重新执行 Bootstrap 原子更新三个角色密码，再重启 API 和 Worker。

管理员凭据只用于连接维护数据库和执行初始化。API、Worker 和 Migrator 完成配置解析后会从进程环境移除 `db_username` 和 `db_password` 等原始字段，只保留自身所需的受限角色 DSN。
...
3. 创建或校正 `dayorder_migrator`、`dayorder_api`、`dayorder_worker`，设置最小权限属性、连接数限制、UTC 时区和超时。
...
6. 使用派生出的 Migrator DSN 对两个数据库执行现有 1–7 号 forward-only migration。
